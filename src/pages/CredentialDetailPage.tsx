...
export const CredentialDetailPage: React.FC = () => {
  ...
  const handleCopyCommitment = (commitment: string) => {
    copyToClipboard(commitment);
    success('Commitment Copied', 'Cryptographic commitment copied to clipboard');
  };

  return (
    <WalletGuard
      featureName="Credential Details"
      description="Connect your Midnight wallet to inspect private credential attributes and construct zero-knowledge proofs."
    >
      {!credential ? (
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10 space-y-8">
          ...
          <div className="flex items-center justify-between">
            ...
            <Badge variant={credential.publicData.status === 'revoked' ? 'error' : 'success'} size="md">
              {credential.publicData.status === 'revoked' ? 'Revoked by Issuer' : '✓ Verified & Active'}
            </Badge>
          </div>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-6">
            <div>
              <span className="text-xs font-mono uppercase text-brand-purple tracking-wider font-semibold">
                {credential.publicData.category} Credential Details
              </span>
              <h1 className="text-2xl sm:text-3xl font-extrabold text-white tracking-tight mt-1">
                {credential.publicData.title}
              </h1>
              <p className="text-xs text-slate-400 mt-1">
                Issued by <strong className="text-slate-300">{credential.publicData.issuerName}</strong>
              </p>
            </div>
                ...
                <Card variant="elevated" className="p-6 border-slate-800 space-y-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                    <div className="p-3 rounded-xl bg-midnight-950 border border-slate-800">
                      ...
                      <span className="text-slate-200 font-semibold mt-0.5 block">
                        {credential.publicData.degreeType}
                      </span>
                    </div>
                    ...
                    <div className="p-3 rounded-xl bg-midnight-950 border border-slate-800">
                      ...
                      <span className="text-slate-200 font-semibold mt-0.5 block">
                        {credential.publicData.issueYear} ({formatDate(credential.publicData.issueDate)})
                      </span>
                    </div>
                    ...
                    <div className="p-3 rounded-xl bg-midnight-950 border border-slate-800">
                      ...
                      <span className="text-slate-200 font-mono text-[11px] mt-0.5 block truncate">
                        {credential.publicData.issuerAddress}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="p-3.5 rounded-xl bg-midnight-950 border border-slate-800 space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        onClick={() => handleCopyCommitment(credential.publicData.credentialCommitment)}
                    ...
                    <code className="text-xs text-slate-300 font-mono break-all block bg-midnight-900 p-2.5 rounded-lg border border-slate-800">
                      {credential.publicData.credentialCommitment}
                    </code>
                  </div>
                </Card>
                ...
                <Card variant="elevated" className="p-6 border-slate-800 space-y-5">
                  ...
                  <div className="space-y-3 text-xs">
                    <div className="p-3 rounded-xl bg-midnight-950 border border-slate-800 flex items-center justify-between">
                      ...
                      {isLocallyRevealed ? (
                        <span className="font-semibold text-slate-100">{credential.privateData.fullName}</span>
                      ) : (
                    ...
                    <div className="p-3 rounded-xl bg-midnight-950 border border-slate-800 flex items-center justify-between">
                      ...
                      {isLocallyRevealed ? (
                        <span className="font-semibold text-slate-100">{credential.privateData.studentId}</span>
                      ) : (
                        <span className="font-mono text-brand-purple font-bold tracking-wider">
                          ████████
                        </span>
                    ...
                    <div className="p-3 rounded-xl bg-midnight-950 border border-slate-800 flex items-center justify-between">
                      ...
                      {isLocallyRevealed ? (
                        <span className="font-semibold text-slate-100">{credential.privateData.dateOfBirth}</span>
                      ) : (
                        <span className="font-mono text-brand-purple font-bold tracking-wider">
                          ████████
                        </span>
                    ...
                    <div className="p-3 rounded-xl bg-midnight-950 border border-slate-800 flex items-center justify-between">
                      ...
                      {isLocallyRevealed ? (
                        <span className="font-semibold text-slate-100">
                          {credential.privateData.exactCgpa} / {credential.privateData.cgpaScale}
                        </span>
                      ) : (
                        <span className="font-mono text-brand-purple font-bold tracking-wider">
                          ████ Private
